...
/* ===========================
 *  Add Users Modal (wired)
 * =========================== */
const AddUsersModal = ({ open, onClose, territoryName, territoryId }) => {
  ...
  const [assignUsers, { isLoading: isSaving } ] = useAssignUsersToTerritoryMutation();
  ...
  const { data: usersData = {}, isLoading, isError, error } = useGetLeadUsersQuery({ page, perPage: 20, search: debounced });

  const users = usersData?.data?.data || [];
  const meta = usersData?.data?.meta ?? usersData?.meta;
  ...
  const handleSave = async () => {
    try {
      ...
      if (!territoryId || user_ids.length === 0) return;

      const res = await assignUsers({ territoryId, user_ids }).unwrap();
      setServerMsg(res?.message || "Users assigned successfully.");
      setTimeout(() => onClose?.(), 600);
    } catch (e) {
      setServerError(e?.data?.message || "Failed to assign users.");
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center">
      ...
      <div className="relative w-full max-w-2xl mx-auto bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-700">
          <div>
            <h3 className="text-gray-100 font-semibold">Add Users</h3>
            <p className="text-xs text-gray-400">{territoryName ? `Territory: ${territoryName}` : territoryId ? `ID: ${territoryId}` : null}</p>
          </div>
          <button onClick={() => onClose?.()} className="p-2 rounded-lg hover:bg-gray-800 text-gray-300" aria-label="Close">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="px-5 pt-4">
          <div className="flex items-center gap-2 bg-gray-800 border border-gray-700 rounded-xl px-3 py-2">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search users by name or email…" className="bg-transparent outline-none text-sm text-gray-200 placeholder:text-gray-500 w-full" />
          </div>
        </div>
        ...
        <div className="px-5 py-4 max-h-[420px] overflow-auto">
          {!!serverError && (
            <div className="mb-3 text-sm text-red-300 bg-red-900/20 border border-red-800/50 rounded-xl p-3">{serverError}</div>
          )}
          {!!serverMsg && (
            <div className="mb-3 text-sm text-emerald-300 bg-emerald-900/20 border border-emerald-800/50 rounded-xl p-3">{serverMsg}</div>
          )}

          {isLoading ? (
            <div className="flex items-center justify-center text-gray-400 py-10"><Loader2 className="w-4 h-4 animate-spin mr-2" /> Loading users…</div>
          ) : isError ? (
            <div className="text-red-300 text-sm bg-red-900/20 border border-red-800/50 rounded-xl p-4">Failed to load users. {error?.data?.message ? `(${error.data.message})` : ''}</div>
          ) : users.length === 0 ? (
            <div className="text-gray-400 text-sm bg-gray-800/40 border border-gray-700/50 rounded-xl p-6 text-center">No users found.</div>
          ) : (
            <ul className="space-y-2">
              {users.map((u) => {
                const isChecked = selected.has(u.id);
                return (
                  <li key={u.id} className="flex items-center justify-between gap-3 bg-gray-800/40 hover:bg-gray-800/60 border border-gray-700/50 rounded-xl p-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2">
                        <UsersIcon className="w-4 h-4 text-gray-400" />
                        <p className="text-sm text-gray-100 truncate">{u.name || u.email || `User #${u.id}`}</p>
                      </div>
                      {u.email ? <p className="text-xs text-gray-400 truncate ml-6">{u.email}</p> : null}
                    </div>
                    <label className="inline-flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" className="h-4 w-4 rounded border-gray-600 bg-gray-800 text-indigo-600 focus:ring-0" checked={isChecked} onChange={() => toggleUser(u.id)} />
        ...
        <div className="flex items-center justify-between px-5 py-4 border-t border-gray-700">
          {/* Pagination */}
          <div className="flex items-center gap-2">
            <button onClick={() => setPage(1)} disabled={page <= 1 || isLoading} className="px-3 py-2 text-xs rounded-lg border border-gray-700 bg-gray-800/60 text-gray-300 disabled:opacity-50"><ChevronsLeft className="w-4 h-4" /></button>
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1 || isLoading} className="px-3 py-2 text-xs rounded-lg border border-gray-700 bg-gray-800/60 text-gray-300 disabled:opacity-50"><ChevronLeft className="w-4 h-4" /></button>
            <span className="text-xs text-gray-400">Page <span className="text-gray-200">{page}</span> / <span className="text-gray-200">{totalPages}</span></span>
            <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page >= totalPages || isLoading} className="px-3 py-2 text-xs rounded-lg border border-gray-700 bg-gray-800/60 text-gray-300 disabled:opacity-50"><ChevronRight className="w-4 h-4" /></button>
            <button onClick={() => setPage(totalPages)} disabled={page >= totalPages || isLoading} className="px-3 py-2 text-xs rounded-lg border border-gray-700 bg-gray-800/60 text-gray-300 disabled:opacity-50"><ChevronsRight className="w-4 h-4" /></button>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-4 py-2 text-sm rounded-lg border transition ${
                isSaving || selected.size === 0
                  ? 'bg-indigo-600/40 text-indigo-200 border-indigo-600/40 cursor-not-allowed'
            ...
            <button onClick={() => onClose?.()} className="px-4 py-2 text-sm rounded-lg bg-gray-800 text-gray-200 border border-gray-700 hover:bg-gray-700">Close</button>
          </div>
        </div>
      </div>
    </div>
...
export default function ItineraryGrid({ onView, onEdit, onCreate }) {
  ...
  const [addUsersForId, setAddUsersForId] = useState(null);
  const [addUsersForName, setAddUsersForName] = useState("");
  ...
  return (
    <div className="min-h-screen bg-gray-900">
      ...
      <AddUsersModal
        open={!!addUsersForId}
        territoryId={addUsersForId}
        territoryName={addUsersForName}
        onClose={() => {
          setAddUsersForId(null);
          setAddUsersForName("");
        }}
      />
    </div>
